...
func ValidateSkillName(name string) error {
	...
	if name == "all-skills" {
		return fmt.Errorf("\"all-skills\" is a reserved name used by the bundled all-skills marketplace plugin")
	}
	return nil
}
...
func (s *RDBConfigStore) UpdateSkill(ctx context.Context, skill *tables.TableSkill, version string, serve bool, objectStore objectstore.ObjectStore) error {
	...
	if err := s.DB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Model(&tables.TableSkill{}).
			Where("id = ?", skill.ID).
			Select("Description", "License", "Compatibility", "Metadata", "ExtraFrontmatter", "AllowedTools", "SkillMDBody", "LatestVersion", "ConfigHash", "UpdatedAt").
			Updates(skill)
		if result.Error != nil {
			return result.Error
		}
