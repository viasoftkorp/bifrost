...
func migrationAddCreatedByUserIDColumnForVirtualKeys(ctx context.Context, db *gorm.DB) error {
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		...
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			if err := tx.Exec("ALTER TABLE governance_virtual_keys ADD COLUMN IF NOT EXISTS created_by_user_id VARCHAR(255)").Error; err != nil {
				return fmt.Errorf("failed to add created_by_user_id column to governance_virtual_keys: %w", err)
			}
			return nil
		},
		Rollback: func(tx *gorm.DB) error {
			if err := tx.Exec("ALTER TABLE governance_virtual_keys DROP COLUMN IF EXISTS created_by_user_id").Error; err != nil {
