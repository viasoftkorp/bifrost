...
func RunSingleMigration(ctx context.Context, options *migrator.Options, db *gorm.DB, logger schemas.Logger, migration *migrator.Migration) error {
	...
	m := migrator.New(db.WithContext(ctx), migrationOpts, logger, []*migrator.Migration{migration})
	return m.Migrate()
}
...
func triggerMigrations(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	...
	if err := migrationAddVirtualKeyBlacklistedModelsColumn(ctx, db, logger); err != nil {
		return err
	}
...
func migrationAddClientConfigMetadataColumn(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	...
	m := migrator.New(db, migrator.DefaultOptions, logger, []*migrator.Migration{{
		ID: migrationName,
	...
	m := migrator.New(db, migrator.DefaultOptions, logger, []*migrator.Migration{{
		...
		Migrate: func(tx *gorm.DB) error {
			...
			if !migrator.HasTable("governance_virtual_key_keys") {
				...
				`
				logger.Info("[configstore] %s: %s", migrationName, "executing createJoinTableSQL).Error; err != nil { return fmt.Errorf(\"failed to create gove")
		...
		Rollback: func(tx *gorm.DB) error {
			logger.Info("[configstore] %s: %s", migrationName, "executing DROP TABLE IF EXISTS governance_virtual_key_keys\").Error; err != nil { return er")
...
func migrationMissingProviderColumnInKeyTable(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	...
	m := migrator.New(db, options, logger, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migrator := tx.Migrator()
...
func migrationAddAllowAllKeysToProviderConfig(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	...
	m := migrator.New(db, &opts, logger, []*migrator.Migration{{
		ID: migrationName,
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			migratorInstance := tx.Migrator()
