...
- [x] **Virtual keys / governance** (`X-Bifrost-VK` header with allowed_models) - covered by `bifrost-v1-vk-quota` (quota endpoint contract), `bifrost-v1-rate-limit` (429 request/token limits, 402 budget), and `bifrost-v1-vk-rotation-cooldown` (rotation grace windows at 1m/3m); allowed_models enforcement in `bifrost-v1-vk-expiry` and `bifrost-routing-wiring`; regex entries (`regex:<pattern>`) in `allowed_models` / `blacklisted_models` / provider key `models` in the `Governance - VK Regex Model Lists` folder of `bifrost-api-management` (400 on unparsable patterns, list-models filtering on both endpoints, 200 for a regex-allowed model, 403 for models outside the allow regex or matching the block regex)
