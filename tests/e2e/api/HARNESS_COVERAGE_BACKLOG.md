...
- [x] **Virtual keys / governance** (`X-Bifrost-VK` header with allowed_models) - covered by `bifrost-v1-vk-quota` (quota endpoint contract), `bifrost-v1-rate-limit` (429 request/token limits, 402 budget), and `bifrost-v1-vk-rotation-cooldown` (rotation grace windows at 1m/3m); allowed_models enforcement in `bifrost-v1-vk-expiry` and `bifrost-routing-wiring`; `allowed_models_patterns` / `blacklisted_models_patterns` on VKs and `models_patterns` / `blacklisted_models_patterns` on provider keys in the `Governance - VK Model Patterns` folder of `bifrost-api-management` (400 on unparsable, empty, wildcard or duplicate patterns, list-models filtering on both endpoints, 200 for a pattern-allowed model, 403 for models outside the allow side or matching a block pattern, a regex-looking string in `allowed_models` stays an exact name, a 13-scenario allow/block matrix, PUT update flow and provider-key round-trip)
