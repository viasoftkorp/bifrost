...
func TestBuildPassthroughURL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got := provider.buildPassthroughURL(nil, makeKey(endpoint), tt.path, tt.rawQuery)
			if got != tt.want {
				t.Errorf("\ngot:  %s\nwant: %s", got, tt.want)
			}
		})
...
func TestBuildPassthroughURL_AliasAPIVersionOverride(t *testing.T) {
	...
	t.Run("deployments route: alias APIVersion overrides default", func(t *testing.T) {
		got := provider.buildPassthroughURL(ctx, makeKey, "/openai/deployments/gpt-4o/chat/completions", "")
	...
	t.Run("responses route: alias APIVersion overrides preview default", func(t *testing.T) {
		got := provider.buildPassthroughURL(ctx, makeKey, "/openai/v1/responses", "")
	...
	t.Run("caller-supplied api-version wins over alias override", func(t *testing.T) {
		got := provider.buildPassthroughURL(ctx, makeKey, "/openai/deployments/gpt-4o/chat/completions", "api-version=2023-01-01")
