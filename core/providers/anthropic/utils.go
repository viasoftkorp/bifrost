...
// resolveAnthropicProgrammaticCaller decides what OpenAI's "programmatic" caller
// becomes for this request. allowed_callers has to name the same code execution
// version the request declares: Anthropic auto-injects a code_execution tool for the
// caller it sees, so a version mismatch 400s on a tool-name conflict. Returns the
// caller value ("" when none can be expressed) and the version the code execution
// tool must be emitted as ("" to leave it as is).
