...
function BindingCell({ row }: { row: OAuth2GrantRow }) {
	...
	const truncated = display.length > 28 ? display.slice(0, 14) + "..." + display.slice(-8) : display;
	if (row.bf_mode === "user") {
		return (
	...
	return (
		<span className="inline-flex items-center gap-2">
			...
			<span className="text-muted-foreground text-sm italic">Anonymous session</span>
		</span>
	);
}
