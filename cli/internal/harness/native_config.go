...
func replaceTOMLTable(data []byte, name string, body []string) []byte {
	...
	skipping := false
	for _, line := range lines {
		if tomlTableHeaderMatches(line, header) {
			skipping = true
			continue
		}
		if skipping {
...
// tomlTableHeaderMatches reports whether line declares exactly the given
// [table] header, tolerating a trailing inline comment (e.g.
// "[model_providers.bifrost] # custom provider").
func tomlTableHeaderMatches(line, header string) bool {
