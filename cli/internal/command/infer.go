...
func (r *Runner) runInfer(ctx context.Context, env *environment, args []string) error {
	if len(args) == 0 || args[0] == "--help" || args[0] == "-h" {
		...
		fmt.Fprintf(r.Out, "Usage: bifrost infer <operation> --body JSON|--file PATH [--header 'Name: value'] [--stream]\n\nOperations:\n  %s\n", strings.Join(operations, "\n  "))
		return nil
	}
	operation := args[0]
...
func (r *Runner) runHarness(ctx context.Context, env *environment, args []string) error {
	if len(args) == 0 || args[0] == "--help" || args[0] == "-h" {
		fmt.Fprint(r.Out, "Usage: bifrost run <claude|codex|gemini|opencode> [--model MODEL] [--worktree NAME] [--skip-verify] [-- agent arguments]\n")
		return nil
	}
