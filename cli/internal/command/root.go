...
var commandSpecs = map[string]commandSpec{
	...
	"status": {
		credentials: fixedCredentials(credentialsManagement),
		handler:     func(r *Runner, ctx context.Context, env *environment, _ []string) error { return r.runStatus(ctx, env) },
	},
	"doctor": {
		credentials: fixedCredentials(credentialsAll),
		handler:     func(r *Runner, ctx context.Context, env *environment, _ []string) error { return r.runDoctor(ctx, env) },
	},
	"request": {
...
func (r *Runner) Run(ctx context.Context, args []string) error {
	...
	if supportsOfflineHelp(name) && isHelpRequest(commandArgs) && !(name == "usage" && len(commandArgs) == 0) {
