...
func (r *Runner) callOperation(ctx context.Context, env *environment, args []string) error {
	...
	fs.BoolVar(&yes, "yes", false, "confirm DELETE operations")
	...
	if strings.EqualFold(operation.Method, "DELETE") && !yes && !dryRun {
		return fmt.Errorf("refusing DELETE operation without --yes; inspect it first with --dry-run")
	}
	if dryRun {
