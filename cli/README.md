...
When both an SSO session and a legacy raw virtual key are configured, SSO takes
precedence for inference. The raw key remains a fallback for contexts without a
signed-in session. An explicit `x-bf-vk` or `Authorization` header supplied to
`bifrost request` is still treated as an intentional per-request override.
