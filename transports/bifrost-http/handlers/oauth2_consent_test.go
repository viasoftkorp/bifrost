...
func TestConsentFlowSubmit_VK(t *testing.T) {
	activeVK := &configtables.TableVirtualKey{ID: "vk-row-1", Value: "sk-bf-active", IsActive: boolPtr(true)}
	inactiveVK := &configtables.TableVirtualKey{ID: "vk-row-2", Value: "sk-bf-inactive", IsActive: boolPtr(false)}
	...
	t.Run("inactive VK is rejected", func(t *testing.T) {
		store := newConsentStore()
		store.vksByValue["sk-bf-inactive"] = inactiveVK
		seedPendingFlow(store, "flow-1", time.Now().Add(time.Minute))
		h := newConsentHandler(store, nil, false)
...
func TestConsentFlowSubmit_VKUserBinding(t *testing.T) {
	boundVK := &configtables.TableVirtualKey{ID: "vk-row-1", Value: "sk-bf-bound", IsActive: boolPtr(true)}

	t.Run("bound VK upgrades to user when logged-in user matches", func(t *testing.T) {
		store := newConsentStore()
		store.vksByValue["sk-bf-bound"] = boundVK
		seedPendingFlow(store, "flow-1", time.Now().Add(time.Minute))
