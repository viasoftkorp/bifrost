...
// bf_mode=vk      → BifrostContextKeyGovernanceVirtualKeyID (+ BifrostContextKeyVirtualKey for compatibility)
// bf_mode=session → BifrostContextKeyMCPSessionID
func injectJWTContext(bifrostCtx *schemas.BifrostContext, claims *jwtMCPClaims, vk *configtables.TableVirtualKey, sessionValidated bool) error {
	sub := claims.Subject
	if sub == "" {
	...
	case schemas.MCPAuthModeVK:
		...
		bifrostCtx.SetValue(schemas.BifrostContextKeyGovernanceVirtualKeyID, vk.ID)
		// Set the raw value as well for governance plugin compatibility —
		// governance reads BifrostContextKeyVirtualKey to resolve budget/rate-limit rows.
	...
	bifrostCtx.SetValue(schemas.BifrostContextKeyOAuth2JWTAuthenticated, true)
	// Set the session-validated flag on the same context as the identity keys
	// above, rather than relying on it being copied in from the request context.
	// The resolver reads this to decide whether a new upstream per-user flow may
	// be initiated for a user-mode JWT.
	if sessionValidated {
		bifrostCtx.SetValue(schemas.BifrostContextKeyOAuth2JWTSessionValidated, true)
	}
	return nil
}
