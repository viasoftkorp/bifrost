...
func (h *ConfigHandler) updateConfig(ctx *fasthttp.RequestCtx) {
	...
	if payload.FrameworkConfig.MCPLibraryURL != nil && *payload.FrameworkConfig.MCPLibraryURL != "" && *payload.FrameworkConfig.MCPLibraryURL != modelcatalog.DefaultMCPLibraryURL {
		...
		resp, err := urlCheckClient.Get(*payload.FrameworkConfig.MCPLibraryURL)
		if err != nil {
			logger.Warn("failed to check the accessibility of the MCP library URL: %v", err)
			SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to check the accessibility of the MCP library URL: %v", err))
			return
		}
		defer resp.Body.Close()
		if resp.StatusCode != http.StatusOK {
			logger.Warn("failed to check the accessibility of the MCP library URL: %v", resp.StatusCode)
			SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to check the accessibility of the MCP library URL: %v", resp.StatusCode))
			return
		}
	}
