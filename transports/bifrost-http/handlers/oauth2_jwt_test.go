	...
	return &lib.Config{
		...
		ClientConfig: &configstore.ClientConfig{
			...
			OAuth2ServerConfig: &configtables.OAuth2ServerConfig{
				IssuerURL:      schemas.NewEnvVar(testIssuer),
...
func TestInjectJWTContext(t *testing.T) {
	...
	t.Run("user mode sets user id and authenticated flag", func(t *testing.T) {
		...
		assert.Equal(t, true, bc.Value(schemas.BifrostContextKeyOAuth2JWTAuthenticated))
		assert.Nil(t, bc.Value(schemas.BifrostContextKeyOAuth2JWTSessionValidated))
	})

	t.Run("user mode with validated session sets the session-validated flag", func(t *testing.T) {
		bc := schemas.NewBifrostContext(context.Background(), time.Time{})
		err := injectJWTContext(bc, &jwtMCPClaims{
			RegisteredClaims: jwt.RegisteredClaims{Subject: "user-1"}, BfMode: "user",
		}, nil, true)
		require.NoError(t, err)
		assert.Equal(t, true, bc.Value(schemas.BifrostContextKeyOAuth2JWTSessionValidated))
	})

	t.Run("vk mode sets governance vk id and raw value", func(t *testing.T) {
		bc := schemas.NewBifrostContext(context.Background(), time.Time{})
		err := injectJWTContext(bc, &jwtMCPClaims{
			RegisteredClaims: jwt.RegisteredClaims{Subject: "vk-row-1"}, BfMode: "vk",
		}, activeVK, false)
		require.NoError(t, err)
		assert.Equal(t, "vk-row-1", bc.Value(schemas.BifrostContextKeyGovernanceVirtualKeyID))
	...
	t.Run("missing sub errors", func(t *testing.T) {
		bc := schemas.NewBifrostContext(context.Background(), time.Time{})
		err := injectJWTContext(bc, &jwtMCPClaims{BfMode: "user"}, nil, false)
		require.Error(t, err)
	})
