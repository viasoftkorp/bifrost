...
func (h *OAuth2IssuanceHandler) handleAuthorize(ctx *fasthttp.RequestCtx) {
	...
	if resource == "" {
		redirectError("invalid_request", "resource parameter is required (RFC 8707)")
		return
	}
	...
	// audience to it, so reject any other resource here rather than minting a
	// grant whose audience can never be honored.
	if resource != oauth2MCPResourceURL(ctx, h.store) {
