...
func (s *BifrostHTTPServer) OnKeyAdded(ctx context.Context, provider schemas.ModelProvider, key schemas.Key) error {
	...
	keys, _ := s.Config.GetProviderKeysRaw(provider)
...
// per-(provider, keyID) entries. Keyless providers (no configured keys) fall
// back to a single call with no KeyID, stored under the "" sentinel.
//
...
func (s *BifrostHTTPServer) FetchAndStoreLiveForKey(ctx context.Context, provider schemas.ModelProvider, keyID string) {
	...
	bfCtx.SetValue(schemas.BifrostContextKeyValidateKeys, true)
	defer bfCtx.Cancel()
