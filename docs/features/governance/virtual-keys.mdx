...
**Content logging** (optional): **Inherit gateway setting** (default), **Off for this key**, or **On for this key**. See [Content logging per key](#content-logging-per-key).
...
### Content logging per key

Each virtual key can decide whether the content of its requests and responses is stored. The `disable_content_logging` field is tri-state:

- omitted or `null`: inherit `client.disable_content_logging`
- `true`: content is not stored in the log store for this key's traffic (an allowed `x-bf-disable-content-logging: false` header can still re-enable it for one request), and OpenTelemetry export is stripped of content too
- `false`: content is stored for this key's traffic even when the client setting is `true`
...
curl -X POST http://localhost:8080/api/governance/virtual-keys \
  ...
  -d '{
    ...
    "provider_configs": [{ "provider": "openai", "allowed_models": ["*"], "key_ids": ["*"] }]
  }'

# Clear the decision back to inherit
curl -X PUT http://localhost:8080/api/governance/virtual-keys/{vk_id} \
  -H "Content-Type: application/json" \
  -d '{ "disable_content_logging": null }'
```

Precedence is client setting, then the key, then the `x-bf-disable-content-logging` header when per-request overrides are allowed. See [Content Logging](/features/observability/content-logging) for how each layer treats the log store and observability connectors.
...
- An entry in either list that starts with `regex:` is an RE2 pattern instead of a name. It must match the whole model name and is case-insensitive, so `"allowed_models": ["gpt-4o", "regex:^claude-3-.*"]` allows `gpt-4o` and every `claude-3-` model, and `"blacklisted_models": ["regex:.*-preview$"]` blocks every preview. A pattern that is empty, `*`, or does not compile is refused with 400. Patterns in `blacklisted_models` win over `allowed_models`, and patterns are never listed as models.
...
Virtual keys can optionally carry an expiry timestamp. Once the expiry passes, requests using the key are rejected with a `403` and the reason `Virtual key has expired`. By default the key is not deleted or deactivated, so it stays visible for auditing and can be restored at any time.
...
- **Delete after expire** (optional) — tick **Delete after expire** (API: `delete_after_expire: true`) and a cleanup job that runs once a day removes the key after it expires. Each run that deletes at least one key posts a notification listing the removed keys; runs that find nothing stay silent. The flag needs an expiry: setting it without one returns `400`, and clearing the expiry resets it.
...
3. Optionally tick **Delete after expire** to have the key removed automatically within 24 hours of expiring

![Virtual Key Expiry Picker](../../media/ui-virtual-key-expiry.png)

Expired keys show an **Expired** badge in the virtual keys table, with **auto-delete** appended when the key is marked for cleanup.
...
Timestamps must be RFC3339 and in the future; otherwise the API returns `400`. On update, omitting `expires_at` leaves the current expiry unchanged. `delete_after_expire` can be sent on create or update; it is rejected with `400` when the key has no expiry.
