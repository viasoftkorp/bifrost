...
#### Allowing or blocking model families with regex

`allowed_models` and `blacklisted_models` accept regex entries alongside model names. Prefix an entry with `regex:` and Bifrost compiles the rest as an [RE2](https://github.com/google/re2/wiki/Syntax) pattern, matched case-insensitively as a full match against the model name and against `provider/model`. One entry then covers a whole family:

```json
{
  "provider": "openai",
  "allowed_models": ["regex:^gpt-4.*", "o3"],
  "blacklisted_models": ["regex:.*-preview$"]
}
```

- A pattern has to match the whole name. `regex:gpt-4` matches `gpt-4` only; `regex:gpt-4.*` matches `gpt-4o` and `gpt-4o-mini`.
- The denylist still wins. In the example, `gpt-4o-preview` is blocked even though the allowlist pattern matches it.
- `["*"]` keeps its meaning and cannot be mixed with other entries, patterns included.
...
- An entry that does not compile is rejected when the key is saved.

The same syntax works on provider key `models` and `blacklisted_models`, and on access profile and project provider configurations. In the web UI, switch the model picker from **Models** to **Regex** to add a pattern.
