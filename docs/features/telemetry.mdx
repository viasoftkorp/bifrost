...
| `bifrost_error_requests_total`     | Counter   | Total failed requests, by returned status and normalized reason    | Base Labels, `status_code`, `error_type`, custom labels |
...
- `app`: Calling client classified from the User-Agent (`Claude Code`, `Cursor`, `Codex CLI`, ..., `API`, or `Other`); empty when no User-Agent was sent
...
`user_id` / `user_name` are not Base Labels. They are opt-in via the telemetry plugin's `user_labels_enabled` config field, because user identity has unbounded cardinality and multiplies series count. See [Prometheus → User Labels](/features/observability/prometheus#user-labels).
...
These runtime dimensions are also forwarded to the other observability backends. The same `x-bf-dim-*` values appear in internal logs, OpenTelemetry span attributes, Datadog spans, and Maxim tags.
...
- alert: BifrostHighErrorRate
  # Excludes caller_* (bad model names, malformed requests) and policy_* (governance
  # refusals: budgets, rate limits, blocked models). Neither is a provider or gateway
  # failure, and policy refusals short-circuit before any provider call while still
  # counting in bifrost_upstream_requests_total — so without this they inflate both
  # sides of the ratio. provider_* and bifrost_* are what this alert is for. See the
  # error_type reference: /features/observability/prometheus#error-types
  expr: sum by (provider) (rate(bifrost_error_requests_total{error_type!~"(caller|policy)_.*"}[5m])) / sum by (provider) (rate(bifrost_upstream_requests_total[5m])) > 0.05
  for: 2m
  labels:
    severity: warning
  annotations:
