...
| `bifrost_provider_key_healthy`     | Gauge     | Per-key health: `1` after a successful attempt, `0` after a failure | `provider`, `key_id`, `key_name`                    |
...
| `bifrost_request_retries`          | Histogram | Number of retries used per request (observed once per request; buckets `0,1,2,3,5,10`). Retained for backwards compatibility — prefer the `number_of_retries` label on other Bifrost metrics for new dashboards. | Base Labels |
