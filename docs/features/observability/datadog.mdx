...
| `metric_dimensions` | `string[]` | No | - | `x-bf-dim-*` dimension names to add as metric tags. See [Dimensions in Metrics](#dimensions-in-metrics) |
...
Per-request dimensions sent as `x-bf-dim-*` headers reach Datadog **APM spans automatically** — every span attribute becomes a tag, so `x-bf-dim-team: platform` appears as `team:platform` on the request span with no configuration.

**Metrics are opt-in.** DogStatsD tags are not added unless the dimension is named in `metric_dimensions`:

```json
{
  "metric_dimensions": ["team", "environment"]
}
```

With that config, `x-bf-dim-team: platform` adds `team:platform` to every `bifrost.*` metric. A dimension not listed is ignored for metrics (it still reaches spans).

<Warning>
Dimension names and values are operator-defined and unbounded. Each name added here multiplies metric series by that dimension's distinct values, and every new tag key counts toward Datadog's custom-metric cardinality. List only low-cardinality dimensions — `environment` or `region`, not `user_id` or `request_id`.
</Warning>

<Note>
Prometheus has the same opt-in model via its `custom_labels` setting. Per-user attribution without the cardinality cost is available from BigQuery columns or Splunk event fields.
</Note>
...
| `bifrost.errors.total` | Counter | Failed requests | provider, model, method, status_code, reason |
...
`bifrost.errors.total` carries two error dimensions, mirroring `bifrost_error_requests_total` on the [Prometheus](./prometheus#error-types) and [OTel](./otel) sides.

`status_code` is the HTTP status the caller received. It includes the 500s Bifrost raises internally — a request that fails during conversion or on an unparseable provider response has no upstream status of its own, and is tagged `status_code:500` because that is what the client got. Alert on this.

`reason` is a per-provider vocabulary kept for existing dashboards and is deprecated; it resolves from `bifrost.error.type`, then `gen_ai.error.type`, then `gen_ai.error.code`, then `unknown`. Prefer `status_code`, or the normalized `bifrost.error.type` on the span.

<Note>
Before Bifrost 2.2.2 these internally-raised failures were tagged `status_code:unknown`, so 5xx dashboards under-counted them. If you have alerts or dashboards keyed on `status_code:unknown`, repoint them — that value is now emitted only for a span carrying no status at all.
</Note>
