...
| `export_raw_payloads` | `boolean` | ❌ No | Attach raw provider request/response bodies to LLM spans (default: `false`). See [Raw Provider Payloads](#raw-provider-payloads) |
...
`export_raw_payloads` attaches the **raw bodies Bifrost exchanged with the provider** to LLM spans, as `bifrost.raw_request` and `bifrost.raw_response`. This is what makes the provider's wire format visible in Langfuse or any other OTLP backend.

```json
{
  "export_raw_payloads": true
}
```

Three conditions must all hold, and any one of them left unset means nothing is exported:

1. The provider must capture them — `store_raw_request_response` on the provider config.
2. This profile must opt in — `export_raw_payloads`. Other profiles on the same collector are unaffected.
3. `disable_content_logging` must be `false`. It always wins: raw bodies are content.

<Warning>
Guardrail redaction still applies: literal replacements are applied to raw bodies before export, the same as to normalized message content. What raw bodies add is *surface* — they carry provider-specific fields, and anything a plugin rewrote on the way out, that the normalized view never exposes. Treat them as the broadest export Bifrost offers and enable them only where the collector is trusted with that. Bodies over 256 KB are dropped rather than truncated.
</Warning>

<Note>
Nothing is built unless a connector asks for it. With no profile setting `export_raw_payloads`, the tracer never assembles raw bodies at all, so there is no cost to leaving it off.
</Note>
...
| `bifrost_error_requests_total` | Counter | Error requests, labelled by `status_code` |
...
<Note>
Before Bifrost 2.2.2, a failure Bifrost raised itself — a request-conversion error, or an empty or unparseable provider response — carried no status of its own and was labelled `status_code="unknown"`, even though the caller received a 500. 5xx dashboards and alerts under-counted these. They are now labelled `status_code="500"`, and the counter gained an `error_type` attribute matching the Prometheus one; repoint anything keyed on `status_code="unknown"`. See [Error Types](./prometheus#error-types) for the `error_type` vocabulary.
</Note>
