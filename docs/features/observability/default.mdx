...
| `attempt_trail` | Ordered array of every attempt. Each entry contains `attempt`, `key_id`, `key_name`, `fail_reason` (null only on a successful attempt — set on every failed attempt, including the terminal one), and optional `triggered_rotation` (`true` when this attempt's per-key failure — rate-limit, auth, billing, or permission — caused the next retry to switch to a different key; **omitted from the JSON when false**). |
...
Auth/billing/permission failures (401/402/403) also rotate to a different key, since the failure is bound to the credential. `fail_reason` is `authentication_error` (401/403) or `billing_error` (402):

```json
"attempt_trail": [
  { "attempt": 0, "key_id": "key-a", "key_name": "Key A", "fail_reason": "authentication_error", "triggered_rotation": true },
  { "attempt": 1, "key_id": "key-b", "key_name": "Key B", "fail_reason": null }
]
```
...
```json
"attempt_trail": [
  { "attempt": 0, "key_id": "key-a", "key_name": "Key A", "fail_reason": "invalid_request_error" }
]
```
