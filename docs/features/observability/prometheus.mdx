...
`bifrost_error_requests_total` carries two error dimensions. `status_code` is the status the caller received, including the 500s Bifrost raises internally. `error_type` is the normalized interpretation: a closed, low-cardinality vocabulary that answers **whose fault was it**, which is what alarms actually need.

<Note>
Before Bifrost 2.2.2, a failure Bifrost raised itself — a request-conversion error, or an empty or unparseable provider response — carried no status of its own and was labelled `status_code="unknown"`, even though the caller received a 500. 5xx dashboards and alerts under-counted these. They are now labelled `status_code="500"`; repoint anything keyed on `status_code="unknown"`, which is no longer emitted on this path.
</Note>
...
- `app` - Calling client classified from the User-Agent (`Claude Code`, `Cursor`, `Codex CLI`, `Cline`, `Windsurf`, ..., `API`, or `Other`); empty when no User-Agent was sent
