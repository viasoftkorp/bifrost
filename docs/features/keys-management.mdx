...
<Note>
In Bifrost Enterprise, [Adaptive Load Balancing](/enterprise/adaptive-load-balancing) replaces the static weights with measured ones while key selection is on. Either way, it holds a key the provider refused until one request checks it again, and editing a key releases every hold on it, since the provider's answers applied to the key as it was. See [Held Keys](/enterprise/adaptive-load-balancing#held-keys).
</Note>
...
- **`regex:` entries**: In `models` or `blacklisted_models`, an entry such as `"regex:^gpt-4.*"` is an RE2 pattern matched case-insensitively against the whole model name. Other entries stay exact. A pattern that is empty, `*`, or does not compile is refused with 400
