...
- **`models_patterns` / `blacklisted_models_patterns`**: Optional RE2 patterns that allow or block a whole family, for example `^gpt-4.*`. Patterns are case-insensitive and must match the whole model name; a block pattern wins. See [Allowing or blocking model families with patterns](/features/governance/virtual-keys#allowing-or-blocking-model-families-with-patterns)
