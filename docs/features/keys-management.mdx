...
- **`models_patterns` / `blacklisted_models_patterns`**: Optional RE2 patterns that allow or block a whole family, for example `^gpt-4.*`. Patterns are case-insensitive and must match the whole name, either the model name on its own or `provider/model`; `*` is not a valid pattern, and a block pattern wins over both the exact allowlist and the allow patterns. See [Allowing or blocking model families with patterns](/features/governance/virtual-keys#allowing-or-blocking-model-families-with-patterns)
