...
- **Regex entries**: Prefix an entry in `models` or `blacklisted_models` with `regex:` to cover a family, for example `regex:^gpt-4.*`. Patterns are RE2, case-insensitive, and must match the whole model name. See [Allowing or blocking model families with regex](/features/governance/virtual-keys#allowing-or-blocking-model-families-with-regex)
