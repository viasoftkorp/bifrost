...
| **VirtualMCPs** | Virtual MCP management (authorizes both `/api/mcp/virtual-mcps` and the deprecated `/api/mcp/tool-groups`) |
| **MCPLogs** | MCP tool execution logs |
| **AdaptiveRouter** | Adaptive load balancing: **View** reads the adaptive routing dashboard and the load balancer settings; **Update** changes the settings and releases [held keys](/enterprise/adaptive-load-balancing#releasing-held-keys) |
| **Warp** | Warp configuration and log index backfills. View reads the configuration; Update saves it and runs backfills |
| **WarpSession** | Warp sessions: chat and conversation history (View), and deleting your own conversations (Delete). The chat panel also reads the Warp configuration, so chat users need Warp View too |
