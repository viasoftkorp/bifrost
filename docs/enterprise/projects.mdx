...
| `allowed_models` | An allowlist; `["*"]` means all models. An entry prefixed with `regex:` is an RE2 pattern matched against the whole model name |
| `blacklisted_models` | A denylist, which wins over the allowlist. Accepts `regex:` entries |
...
A list may not mix a wildcard with named entries: `["*", "gpt-4o"]` is refused rather than interpreted. A regex entry that does not compile is refused the same way. See [Allowing or blocking model families with regex](/features/governance/virtual-keys#allowing-or-blocking-model-families-with-regex) for how patterns are matched. Key ids are validated when you save, and a key that does not exist or belongs to a different provider is rejected. A provider may carry up to 100 per-model budgets, each naming a concrete model; the `*` tier belongs on the provider configuration itself.
