...
// Streaming post-processing may run after headers are committed; use
// HTTPTransportResponseHeadersHook for headers that must reach streaming clients.
...
// HTTPTransportResponseHeadersHook runs once at the last safe point before headers
// are committed. Use it for headers that must also be present on streaming responses.
func HTTPTransportResponseHeadersHook(ctx *schemas.BifrostContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponseMetadata) error {
	resp.SetHeader("x-processed-by", "my-plugin")
	return nil
}
...
#### `ctx.CalculateCostBreakdown(resp)` <sup>v1.10.2+</sup>

Returns the same cost as `CalculateCost`, split by category. Use it when your plugin bills, meters or reports input, output, cache read and cache write separately rather than as one number.

```go
func (p *MyPlugin) PostLLMHook(ctx *schemas.BifrostContext, resp *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) (*schemas.BifrostResponse, *schemas.BifrostError, error) {
	cost := ctx.CalculateCostBreakdown(resp) // compute synchronously
	if cost == nil {
		return resp, bifrostErr, nil // nothing billable, or no catalog configured
	}
	go func() {
		p.reportToBilling(cost.TotalCost)
		if cost.InputCostDetails != nil {
			p.reportCacheSavings(cost.InputCostDetails.CachedReadCost, cost.InputCostDetails.CachedWriteCost)
		}
	}()
	return resp, bifrostErr, nil
}
...
**What comes back** - a `*schemas.BifrostCost`, shaped like the token usage it prices. For costs calculated from the catalog, `InputCost + OutputCost + AdditionalCost == TotalCost`, and each side's details sum to that side. When a provider reports its own cost (for example xAI's `cost_in_usd_ticks`), the object may carry only `TotalCost`:

| Field | Type | Description |
|---|---|---|
| `TotalCost` | `float64` | Identical to `ctx.CalculateCost(resp)` for the same response |
| `InputCost` | `float64` | Prompt side |
| `InputCostDetails` | `*schemas.InputCostDetails` | `TextCost`, `AudioCost`, `ImageCost`, `CachedReadCost`, `CachedWriteCost`, `RequestCost` (flat per-request surcharge) |
| `OutputCost` | `float64` | Completion side |
| `OutputCostDetails` | `*schemas.OutputCostDetails` | `TextCost`, `AudioCost`, `ImageCost`, `ReasoningCost`, `CitationCost`, `SearchQueriesCost` |
| `AdditionalCost` | `float64` | Internal sidecar calls with no token category |
| `AdditionalCostDetails` | `*schemas.AdditionalCostDetails` | `GuardrailCost`, `MCPCost`, `SemanticCacheCost`, `RoutingCost` |

Key points:
- **Same pricing resolution as `CalculateCost`** - tiers, batch/priority/flex/fast rates, cache rates, the fallback chain and pricing overrides all apply; the two never disagree on the total
- **Returns `nil`** when no catalog is configured or the response has no billable usage - always nil-check, and nil-check each `*Details` pointer too, since a side with no cost carries no details
- **Fresh copy** - the returned value belongs to you; mutating it never touches the response the client receives
- **Cache costs live on the input side** - `CachedReadCost` and `CachedWriteCost` sit under `InputCostDetails`, mirroring `PromptTokensDetails.CachedReadTokens` / `CachedWriteTokens` on the usage object
- **Provider-priced responses** - when a provider reports its own cost, the breakdown is whatever the provider supplied; treat a zero side as "not itemised", not as free

<Warning>
The same rule as `CalculateCost` applies: call it **synchronously inside your hook** and close over the result if you need it in a goroutine.
</Warning>

<Note>
All three methods are inert rather than fatal when no model catalog is wired - `GetModelInfo` returns `nil`, `CalculateCost` returns `0` and `CalculateCostBreakdown` returns `nil`. This keeps plugins portable between the Bifrost HTTP gateway, which always configures a catalog, and Go SDK embeddings of `core`, which may not.
</Note>
...
- Do not use it to set streaming response headers; those may already be committed by the time deferred streaming post-processing runs

#### `HTTPTransportResponseHeadersHook(ctx, req, resp)`

**HTTP transport only.** Runs once at the last safe point before response headers are committed. It works for both buffered and streaming responses and executes in reverse plugin registration order.

Key points:
- Receives `*HTTPRequest` and body-free `*HTTPResponseMetadata`
- `resp.StatusCode` is informational; use `resp.SetHeader`, `resp.Header`, and `resp.DeleteHeader` to modify headers
- `Content-Length`, `Transfer-Encoding`, and `Connection` remain controlled by the transport
- The metadata is pooled; do not retain it after the hook returns
- For native `.so` plugins, exporting this symbol is optional and older binaries continue to load
- Direct Go implementations of `schemas.HTTPTransportPlugin` must implement the method; return `nil` for a no-op
...
<Warning>
Use `HTTPTransportResponseHeadersHook` for streaming headers and `HTTPTransportStreamChunkHook` for streaming payload data. A chunk hook runs after the HTTP headers have already been committed.
</Warning>
