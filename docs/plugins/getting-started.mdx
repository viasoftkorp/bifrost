...
<Tabs>
  <Tab title="v1.4.x+">
    ...
    - `HTTPTransportResponseHeadersHook()` - Modify response headers at the pre-commit boundary, including streaming responses (HTTP transport only)
    - `HTTPTransportStreamChunkHook()` - Modify or filter streaming payload chunks (HTTP transport only)
...
<Tabs>
  <Tab title="v1.4.x+">
    ...
    8. `HTTPTransportResponseHeadersHook` - Finalize response headers before they are committed, including for streams (HTTP transport only, reverse order)
    9. `HTTPTransportStreamChunkHook` - Intercept each streaming payload chunk (HTTP transport only, reverse order)
  </Tab>
  <Tab title="v1.3.x">
