...
When the caller asks for a bare model (`gpt-4o`) and routing offers more than one provider for it, the route that last served the session, provider and model together, is tried first, as long as routing still offers that exact route. A chain that offers the same provider on a different model does not offer it: the binding is dropped and the routing decision stands. Routing engines build the chain of providers; affinity reorders that chain, and nothing else. A provider that routing excluded, for budget, rate limits, model allowance, or health, is never brought back by a session.
...
| Served by a fallback provider                          | Moved to the fallback provider                                         | The key the primary followed is deleted; moved to the key the fallback used |
| Failed after following the binding                     | Deleted, the next request follows the routing decision                 | Deleted, a key is picked normally on the next request                |
| Cancelled by the caller after following the binding    | Unchanged, the failure is not the provider's                           | Unchanged                                                            |
| Request failed on every attempt                        | Untouched                                                              | Untouched                                                            |
| Request refused by governance before routing           | Untouched                                                              | Untouched                                                            |
| Bound route not in the chain routing built (provider and model) | Deleted, the routing decision stands, the next served request rebinds  | Unchanged                                                            |
...
| Fallbacks and retries                 | Which provider to try next, and how often to retry                               | Fallback attempts pick their keys freely unless the rule pinned a key for that entry. A pin belongs to the provider it was decided for, so when the session moves that provider elsewhere in the chain the pin moves with it. Retries of one attempt reuse the bound key. Whatever finally serves becomes the session's new binding at both levels.            |
...
- **Key level**: if the load balancer marks the bound key's route as failed on this node, holds the key because the provider refused it (see [Held Keys](/enterprise/adaptive-load-balancing#held-keys)), or the circuit breaker holds the key back, the binding is dropped and a key is picked normally. The next served request rebinds the session to the key that served it.

Bindings live in the shared KV store and replicate across the cluster, while health is measured per node. A node that sees a provider failing drops the binding for every node. A node that has not observed the failure yet still follows the binding until its own load balancer marks the route as failed or the request fails there. A key held because the provider refused it is the exception: the hold reaches every node, so each one drops the binding before sending the key another request.
...
| Trail entry                                                                                                                          | Meaning                                                                                              |
| ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| `Session stays on azure/gpt-4o for gpt-4o; routing proposed openai/gpt-4o`                                                          | The bound route was moved ahead of the one routing put first.                                        |
| `Session stays on azure/gpt-4o for gpt-4o, which routing also proposed`                                                              | The bound route was already first, so the chain was left as it is.                                   |
| `Session reused key prod-eu-2 for azure/gpt-4o`                                                                                      | The bound key was used instead of running key selection.                                             |
| `Session was last served by azure/gpt-4o for gpt-4o, which this request cannot use, so the routing decision stands and the session rebinds on its next served request` | Routing no longer offers the bound route, provider and model together, so the binding was deleted. |
| `Session moved the request from vertex to anthropic, so a key is picked normally there; the key pinned for vertex applies if vertex is tried as a fallback` | The session promoted another provider; the routing rule's pin stayed with the provider it was set for, now a fallback entry. |
| `The key this session followed for azure/gpt-4o failed, so the session forgets it and a key is picked normally next time`            | The request followed the key binding and did not serve on it, so the binding was deleted.            |
| `The provider this session followed for gpt-4o failed, so the session forgets it and the next request follows the routing decision`  | The request followed the route binding and failed outright, so the binding was deleted.              |
...
The last two entries are emitted only by Bifrost Enterprise; an open-source deployment never writes them, because it follows a binding until the request itself fails, and only then forgets it, which the two entries before them record.
...
- **Health checks are node-local (Enterprise).** A binding is refused where a node has seen the failure. A node that has not seen it follows the binding until the request fails or its own load balancer catches up. Keys held because the provider refused them are shared with every node, so this lag applies to error rates, not to refusals.
