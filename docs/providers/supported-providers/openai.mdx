...
- Unsupported tool types are silently filtered (only these are supported: `function`, `file_search`, `computer_use_preview`, `web_search`, `mcp`, `code_interpreter`, `image_generation`, `local_shell`, `shell`, `programmatic_tool_calling`, `apply_patch`, `custom`, `web_search_preview`, `tool_search`, `namespace`)
...
| Shell | `response.shell_call_command.added`, `response.shell_call_command.delta`, `response.shell_call_command.done`, `response.shell_call_output_content.delta`, `response.shell_call_output_content.done` |
