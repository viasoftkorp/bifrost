...
4. **Smooth selection**: Concentrate traffic on the best-scoring providers while keeping a small exploration share for the rest, so a recovered provider keeps getting re-probed. A provider counts as failed for a model only when every configured key that can serve the model is failed; a key that has not been tried yet keeps it in rotation
5. **Checking a held provider**: When the provider refused every key and one of them is due a check, one request is sent to that provider first with the healthy providers behind it as fallbacks (see [Held Keys](/enterprise/adaptive-load-balancing#held-keys))
6. **Fallbacks created**: Remaining healthy providers sorted by performance score (descending) are appended as fallbacks after any the request already carries, deduplicated. With the prune toggle on, circuit-broken configured fallbacks are removed first and never re-added
...
6. **Held keys**: Skip keys the provider refused (rejected credential, exhausted quota, no model access, retired model, blocked region, rate limit) until their wait is over, then let one request through to check each. When every key is held the request is sent anyway. See [Held Keys](/enterprise/adaptive-load-balancing#held-keys)
...
| **Held Keys**              | Keys a provider refused are held on every node and checked again by one request |
...
<Frame>
  <img
    src="/media/adaptive-load-balancing/ui-load-balancing-dashboard.png"
...
<Note>
  **Scope & tuning**: Adaptive load balancing operates **per node** (each node routes on its own observed metrics; of the metric signals, only rate-limit/TPM backoffs are shared across nodes, and only within a region, while [held keys](/enterprise/adaptive-load-balancing#held-keys) are shared with every node) and adapts on a **~5-second cycle** — distinct from per-request fallback, which is immediate. Both levels can be toggled independently, and the scoring parameters ship pre-tuned. See [Adaptive Load Balancing](/enterprise/adaptive-load-balancing) for configuration and limitations.
</Note>
...
<Note>
  **Session affinity** sits on top of this order. After PreRequestHook it moves the
  provider that last served the session to the front of the fallback chain. A request
  that names its provider (`openai/gpt-4o`) is never reordered. Inside Level 2 it reuses
  the key that last served the session while that key stays in the pool, including for a
  request that named its provider. Routing decides which providers and keys are offered;
  the session only decides their order. See [Session Affinity](/providers/session-affinity).
</Note>
