...
- **Refresh tokens** rotate on each use and have no fixed expiry. A token is invalidated by rotation, by the bound virtual key or user becoming inactive, by an explicit revoke, or by enabling `enforce_auth_on_inference` (which drops session-mode grants).
