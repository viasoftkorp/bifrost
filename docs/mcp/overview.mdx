...
Per the MCP spec, a server may return an `instructions` string from `initialize` describing how
its tools should be used. Servers like GitHub's, Grafana's and Outline's use it to carry workflow
rules and workspace policy.

By default Bifrost does not forward these. Turn it on with `mcp.tool_manager_config.server_instructions_mode`:

| Mode | Behavior |
|------|----------|
| `off` (default) | Upstream instructions are dropped. |
| `gateway` | Forwarded on the `/mcp` `initialize` response, aggregated and labeled per source server. |
| `all` | As `gateway`, plus injected into chat and responses requests as a system message. |

```json
{
  "mcp": {
    "tool_manager_config": {
      "server_instructions_mode": "gateway"
    }
  }
}
```

Each upstream's text is wrapped in a tagged block naming the server it came from:

```
<mcp_server name="github">
Always call get_me first to understand current user permissions.
</mcp_server>

<mcp_server name="deepwiki">
DeepWiki MCP provides AI-powered documentation…
</mcp_server>
```

Tagged rather than markdown-headed because upstream instructions are usually markdown and often
contain their own headings — a heading delimiter would be indistinguishable from the content it
wraps. Text containing the block tag is neutralized, so one server cannot forge a boundary and
attribute its guidance to another.

The aggregate is scoped to what the caller may see: a request narrowed by `x-bf-mcp-include-clients`,
or served from a `/mcp/<slug>` Virtual MCP endpoint, only receives the instructions of the servers it
can actually reach. Each server's contribution is capped at 4 KB and the whole aggregate at 16 KB,
with an explicit marker when anything is trimmed.

<Warning>
`all` adds tokens to **every** MCP-bearing LLM request and changes the prompt prefix, which can
invalidate provider-side prompt caching. `gateway` only affects the `/mcp` handshake.
</Warning>

Verify what a client receives:
...
**How it works:**
- **What a check does:** a ping (when the server supports one) plus a `tools/list` over the same connection, so one pass covers both liveness and tool-list refresh
- **Adaptive cadence:** once a client is healthy, checks run on its tool sync interval (10 minutes by default). While a client is `unstable` they run every 10 seconds, so recovery is detected quickly
- **Check timeout:** each ping or `tools/list` attempt is bounded at 5 seconds and retried with backoff inside the check
- **Failure threshold:** a single failed check marks the client `unstable`. There is no consecutive-failure counter across checks; the retries inside each check are what absorb an ordinary blip
- **Automatic recovery:** a failed check reconnects the client in the background, so a session the server expired, a dropped SSE stream, or a `stdio` server whose process exited is replaced without anyone intervening. For `http` and `sse` the replacement is dialled before the old connection is dropped, so calls keep flowing throughout. `stdio` and in-process clients close first instead, because two subprocesses would double resource use and clash over anything held exclusively, such as a port bind or a lockfile
- **Tool calls repair too:** a call that fails because the credential was rejected or the session is gone triggers the same reconnect immediately, and usually retries the caller's own request on the healed connection (see [Auth failure recovery](./tool-execution#auth-failure-recovery))
- **`unstable` does not hide tools:** it reflects Bifrost's own checks, never the outcome of real tool calls, and tool calls keep being attempted normally throughout
...
**Configuration:** the cadence is the only knob. Set it per client with `tool_sync_interval` on a `client_configs[]` entry, or globally under the `mcp` section:
...
Both take a Go duration string (`"30s"`, `"10m"`). A bare number is read as **nanoseconds**, so `10` means 10ns rather than 10 minutes, and the global additionally has to be a whole number of minutes. A per-client `0` or unset value inherits the global; an unset global means 10 minutes. See [State reference](./connections) for what each state means and how recovery works per connection mode.

Recovery is automatic, but you can also force a reconnect yourself for any client except one sitting in `needs_reauth`, which needs [reauthorization](./auth/oauth#reauthorization) rather than another redial:
...
The periodic sync is not the only way to pick up a change. `POST /api/mcp/client/{id}/refresh-tools` re-discovers a single client's tools immediately, which is what to reach for after adding or removing a tool on an upstream server rather than waiting out the interval. It applies to every client type — including per-user and other per-call clients, for which it is the only on-demand refresh path — and its result persists and re-syncs the served registry exactly like a periodic tick. See [Manual Tool Refresh](./connecting-to-servers#manual-tool-refresh).
