...
2. **General** - Enter a **Name** (required). Optionally set an **Endpoint slug** (leave blank to derive it from the name), a **Description**, and **Server Instructions**, and toggle **Enabled**. The page you'll be served at is previewed as `/mcp/<slug>`.

<Note>
**Description** is for operators browsing the list. **Server Instructions** is sent to the model. See [Server instructions](#server-instructions) below.
</Note>
...
An MCP server can return an `instructions` string from its `initialize` handshake describing
how its tools should be used. A Virtual MCP inherits those from the servers it draws tools
from, and can add its own on top or replace them entirely.

<Note>
Inherited instructions are only forwarded when `mcp.tool_manager_config.server_instructions_mode`
is `gateway` or `all` — see [Server Instructions](/mcp/overview#server-instructions). A Virtual
MCP's own text follows the same setting.
</Note>

Set the text in **General -> Server Instructions**, then choose how it combines with what the
source servers provide:

| Toggle | Result |
|--------|--------|
| **Use source server instructions** on (default) | The source servers' text, then yours after it |
| **Use source server instructions** off | Only your text |

Turn it off when the Virtual MCP exposes **part** of a server rather than all of it. The
inherited text describes the whole server, so it will tell the model how to use tools this
endpoint deliberately withheld — and name them.

<Warning>
Turning it off drops the inherited text from **every** source server on this endpoint, not just
the one you were narrowing. With three servers where only one is over-broad, you lose the other
two as well.
</Warning>

Each block is labeled with where it came from, so the model can tell a source server's guidance
from yours:
...
<virtual_mcp name="finance-tools">
Read-only lookups only. Never modify production data.
</virtual_mcp>
```

Via the API, the fields are `instructions` and `instructions_mode` (`append` or `replace`):

```bash
curl -X PUT http://localhost:8080/api/mcp/virtual-mcps/$ID \
  -H "Content-Type: application/json" \
  -d '{"instructions":"Read-only lookups only.","instructions_mode":"replace"}'
```
