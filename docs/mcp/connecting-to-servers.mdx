...
<Note>
Alongside the tools, Bifrost captures whatever the server returns in the `instructions` field of
the MCP `initialize` handshake. Whether that text reaches your clients is a separate setting —
see [Server Instructions](/mcp/overview#server-instructions).
</Note>
...
HTTP and SSE MCP connections honor the standard `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` variables from the Bifrost process environment, the same way every other outbound HTTP client in the process does. This is how a deployment with no direct internet route (a locked-down VPC, or a pod behind a corporate egress proxy) reaches a public MCP server. Set the variables on the container or task definition; a `NO_PROXY` entry keeps an internal MCP server on the direct path.

The `network_config.proxy_config` block in `config.json` does not apply to MCP connections. Its `enable_for_*` flags cover inference, SCIM, and API clients only.

The link-local and cloud-metadata block still applies when a proxy is in use: a connection string whose host is a link-local or metadata IP literal is refused before the proxy is contacted. A hostname on the proxied path is handed to the proxy unresolved, so Bifrost itself does not need to resolve public DNS in a proxy-only deployment.
...
**Refresh a client's tools:**
```bash
curl -X POST http://localhost:8080/api/mcp/client/{id}/refresh-tools
```

<Note>
Different job from reconnect: this re-discovers the client's **tool list** from its upstream server instead of recycling the connection, so it applies to every client type — including the per-call ones reconnect returns `400` for. Use it when an upstream server's tools changed and you don't want to wait for the next periodic sync. See [Manual Tool Refresh](#manual-tool-refresh).
</Note>
...
Reconnecting recycles a *connection*. If what actually changed is the upstream server's **tool set** — a tool added, removed, or re-described — the connection is fine and there is nothing to reconnect; the tool list is simply stale until the next periodic sync, up to `tool_sync_interval` (10 minutes by default).

Refresh it on demand instead:
...
Unlike reconnect, this applies to **every** client type. A sticky client is re-listed over its live connection. A per-call server-level client re-resolves its own configured credential and runs a one-shot connect, `tools/list`, disconnect cycle; a per-user client (`per_user_oauth`, `per_user_headers`, `token_exchange`) runs the same cycle using its retained admin discovery credential. For the per-call and per-user clients alike it is the only on-demand refresh path.
...
Open the actions menu (**⋯**) on any row under **MCP Servers** and choose **Refresh tools**. The toast reports how many tools the client is serving afterwards.

</Tab>
<Tab title="Go SDK">

```go
// Re-discover this client's tools now, rather than at the next periodic sync
count, err := client.RefreshMCPClientTools(ctx, "filesystem")
if err != nil {
    log.Printf("Tool refresh failed: %v", err)
} else {
    log.Printf("filesystem is now serving %d tools", count)
}
```
