...
With Enterprise SSO, Bifrost creates temporary Claude settings containing an
`apiKeyHelper` that calls `bifrost auth print-token`. Claude receives a current
opaque Bifrost agent access token without the token being written to its
settings or inherited environment. The helper can refresh a rejected session
and the temporary settings are removed when Claude exits.

With a virtual key, Bifrost configures Claude's bearer credential directly for
the launched child process. Persistent `bifrost configure claude` is
virtual-key-only; it never writes a short-lived Enterprise agent token to
Claude's long-lived settings.

<Note>
Enterprise SSO authorizes user-scoped inference and assigned-key usage. It does
not authorize Bifrost management APIs; administrative CLI commands still need a
scoped management API key.
</Note>

## Authentication Methods

For manual configuration with a Bifrost virtual key, there are two methods:
