...
| `allowed_models` | No | Model allow-list. `["*"]` = all models; `[]` = deny all; `"regex:<pattern>"` entries are RE2 patterns matched case-insensitively against the whole model name |
| `blacklisted_models` | No | Model deny-list, which wins over `allowed_models`. `["*"]` = block all; `[]` or omission = block none; `"regex:<pattern>"` entries are RE2 patterns matched case-insensitively against the whole model name |
...
| `fallbacks` | No | Ordered fallback list if the primary target fails. See [Fallback Fields](#fallback-fields) |
| `ttft_timeout_ms` | No | Time-to-first-token (TTFT) cutoff for streaming requests, 1 to 300000 ms. An attempt with no first token in time is cut off and the next fallback runs; the last attempt is never cut off. See [TTFT cutoff](/providers/routing-rules#time-to-first-token-ttft-cutoff) |
...
| `provider_key_name` | No | Pin a specific provider key by name. Requires `provider`, and is resolved to an internal key ID at load time |

### Fallback Fields

Each entry is either the string `"provider/model"` (use `"provider/"` to keep the incoming model), or an object when it needs to pin a key:

| Field | Required | Description |
|-------|----------|-------------|
| `provider` | Yes | Fallback provider |
| `model` | No | Fallback model. Omit to keep the incoming request's model |
| `provider_key_name` | No | Pin a specific provider key by name, resolved to an internal key ID at load time. Omit for load-balanced key selection |

```json
"fallbacks": [
  "anthropic/claude-sonnet-4-5",
  { "provider": "vertex", "provider_key_name": "vertex-prod" }
]
```

A fallback whose pinned key is missing from that provider's pool is skipped, and the chain moves to the next entry.
    ...
    "projects": [
      {
        ...
        "virtual_mcps": [{ "virtual_mcp_name": "Platform Tools" }]
      }
    ]
  }
}
...
| `virtual_mcps` | No | Virtual MCPs assigned to the project, one `virtual_mcp_name` per entry, resolved on startup. A scoped request can reach each at its `/mcp/<slug>` endpoint. `virtual_mcp_id` is accepted as an alternative and wins when both are set |
...
| `allowed_models` | No | Allowed model names, ignored when `all_models_allowed` is true; `"regex:<pattern>"` entries are RE2 patterns matched case-insensitively against the whole model name |
| `blacklisted_models` | No | Models blocked even if allowed; `["*"]` blocks all; `"regex:<pattern>"` entries are RE2 patterns matched case-insensitively against the whole model name |
