...
| `allowed_models` | No | Model allow-list. `["*"]` = all models; `[]` = deny all. Entries prefixed with `regex:` are RE2 patterns matched against the whole model name |
| `blacklisted_models` | No | Model deny-list, wins over `allowed_models`. `["*"]` = block all. Accepts `regex:` entries |
...
| `allowed_models` | No | Allowed model names, ignored when `all_models_allowed` is true. Entries prefixed with `regex:` are RE2 patterns matched against the whole model name |
| `blacklisted_models` | No | Models blocked even if allowed; `["*"]` blocks all. Accepts `regex:` entries |
