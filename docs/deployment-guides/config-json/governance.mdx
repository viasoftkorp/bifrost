...
| `allowed_models` | No | Model allow-list of exact names. `["*"]` = all models; `[]` = deny all. A list may not mix `*` with named models |
| `blacklisted_models` | No | Model deny-list of exact names, wins over `allowed_models`. `["*"]` = block all. A list may not mix `*` with named models |
| `allowed_models_patterns` | No | RE2 patterns that allow models by name shape, next to `allowed_models`. Case-insensitive full match against the model name and `provider/model`; `*` is not a valid pattern |
