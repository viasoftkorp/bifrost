...
| `allowed_models` | No | Model allow-list of exact names. `["*"]` = all models; `[]` = deny all |
| `blacklisted_models` | No | Model deny-list of exact names, wins over `allowed_models`. `["*"]` = block all |
| `allowed_models_patterns` | No | RE2 patterns that allow models by name shape, next to `allowed_models`. Case-insensitive full match against the model name and `provider/model` |
| `blacklisted_models_patterns` | No | RE2 patterns that block models by name shape; win over the allow side |
...
| `allowed_models_patterns` | No | RE2 patterns that allow models by name shape, ignored when `all_models_allowed` is true. Each is matched case-insensitively as a full match against the model name and against `provider/model`; `*` is not a valid pattern. See [Allowing or blocking model families with patterns](/features/governance/virtual-keys#allowing-or-blocking-model-families-with-patterns) |
| `blacklisted_models_patterns` | No | RE2 patterns that block models by name shape, matched the same way; win over the allow side |
